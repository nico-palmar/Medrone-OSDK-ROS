...
int main(int argc, char** argv)
{
    ...
    io_service.request.mode = osdk::MFIO::Request::MODE_PWM_OUT;
    io_service.request.channel = osdk::MFIO::Request::CHANNEL_0;  // Specify which GPIO channel (0-7)
    io_service.request.init_on_time_us = 20000; // set an 100% duty cycle
    io_service.request.pwm_freq = 200; // (1/200) = 5ms time for PWM
    io_service.request.block = true;  // Non-blocking call (can be set to true if needed)

    // Call the service
    if (io_control_client.call(io_service))
    {
        std::string msg = "PWM Pin requested on";
